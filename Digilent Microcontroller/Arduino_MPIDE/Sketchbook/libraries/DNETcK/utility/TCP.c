...
/****************************************************************************
  Section:
	Function Prototypes added by KeithV for support of DNETcK / DWIFIcK
  ***************************************************************************/
BOOL DNETcK_IsMacConnected(int * pStatus);
...
void TCPInit(void)
{
    ...
    // clear out module level statics
    hCurrentTCP = INVALID_SOCKET;
    hLastTCB = INVALID_SOCKET;
    memset(&MyTCB, 0, sizeof(MyTCB));	
    memset(TCBStubs, 0, sizeof(TCBStubs));
    memset(SYNQueue, 0, sizeof(SYNQueue));
    memset(&MyTCBStub, 0, sizeof(MyTCBStub));	
...
static void SendTCP(BYTE vTCPFlags, BYTE vSendFlags)
{
    ...
    // KeithV added for DNETcK / DWIFIcK
    // If the WiFi MAC goes down, don't attempt to send the MRF24
    // any data as this will abort the MRF24s attempt to reconnect.
    // basically the assumption is that the data won't be sent anyway
    // and we pretend that it did, but the MAC did not and the TCP engine
    // will have to recover on its own just as if the packet was lost on the
    // Ethernet. TCP will drop into retry logic.
    // Oh, experimentally it has been found to work better to do the IPIsTxReady
    // before the DNETcK_IsMacConnected, it may have to do with the processing within
    // the MRF24 in that the connection loss is noticed when I do the WFProcess() command
    // in the IPIsTxReady
    while(!IPIsTxReady())
    {

#if defined(DWIFIcK_WiFi_Network)         
//        if(!WFcK_IsConnected(0, NULL))
//        {
            DWORD tStart = TickGet();

            // hold on for a half ms
            while((TickGet() - tStart) <= (TICKS_PER_SECOND/2000));
//            return;
//       }
#endif
