 ...
 * $Id: $
 *
 *  Functions added by KeithV (Digilent)
 *      InitMACStaticMemory - to clear memory on MACInit
 *      _MacRxAllocCallback - to remove a call to calloc (sEthDNode must be consistent in size with plib in _eth_dcpt_lists.h)
 *      _MacTxAllocCallback - to remove a call to calloc (sEthDNode must be consistent in size with plib in _eth_dcpt_lists.h)
 *
********************************************************************/
#include <string.h>
...
static void*    _MacRxAllocCallback( size_t nitems, size_t size, void* param );
static void*    _MacTxAllocCallback( size_t nitems, size_t size, void* param );

// this must line up with the size of a sEthDNode in eth_descriptors_add_lib.c in plib
// this is so I don't have to da a calloc
typedef unsigned char sEthDNode[0x18]; 
...
static sEthDNode    _rgTxEthDNode[EMAC_TX_DESCRIPTORS+1];       // so we don't use calloc
static int          _iTxEthDNode = 0;                           // how many have we allocated.
...
static sEthDNode    _rgRxEthDNode[EMAC_RX_DESCRIPTORS+1];       // so we don't use calloc
static int          _iRxEthDNode = 0;                           // how many have we allocated.
...
/*****************************************************************************
  Function:
	void InitMACStaticMemory(void)

  Summary:
	Initializes all of MAC static memory to zero
	
  Description:
    This function will clear the MAC module to its just 
    initalized state. The compiler clears all static
    memory and this will restore the static variables to that state
...
void InitMACStaticMemory(void)
{
    ...
    _TxLastDcptIx=0;					
    _TxCurrSize=0;						
    memset(_RxBuffers, 0, sizeof(_RxBuffers));	
    _pRxCurrBuff=0;						
    _RxCurrSize=0;						
    memset(_HttpSSlBuffer, 0, sizeof(_HttpSSlBuffer));
    _CurrWrPtr=0;						
    _CurrRdPtr=0;						
    _linkUpdTick = 0;						
    _linkPrev = 0;				
    ...
    _linkNegotiation = 0;					
    _stackMgrRxOkPkts=0;
    _stackMgrRxBadPkts=0;
    _stackMgrInGetHdr=0;
    _stackMgrRxDiscarded=0;
    _stackMgrTxNotReady=0;

    // junk I added so calloc is not needed.
     memset(_rgTxEthDNode, 0, sizeof(_rgTxEthDNode));
    _iTxEthDNode = 0;                           
     memset(_rgRxEthDNode, 0, sizeof(_rgRxEthDNode));
    _iRxEthDNode = 0;                           
...
void MACInit(void)
{
	...
	while(1)
	{
		...
		if(EthDescriptorsPoolAdd(EMAC_TX_DESCRIPTORS, ETH_DCPT_TYPE_TX, _MacTxAllocCallback, 0)!=EMAC_TX_DESCRIPTORS)
		{
			initFail++;
		}

		if(EthDescriptorsPoolAdd(EMAC_RX_DESCRIPTORS, ETH_DCPT_TYPE_RX, _MacRxAllocCallback, 0)!=EMAC_RX_DESCRIPTORS)
		{
			initFail++;
		}
...
static void* _MacTxAllocCallback( size_t nitems, size_t size, void* param )
{
    if(sizeof(sEthDNode) != size || nitems != 1)
    {
        return(NULL);
    }
    if(_iTxEthDNode < (EMAC_TX_DESCRIPTORS+1))
    {
        return(&_rgTxEthDNode[_iTxEthDNode++]);
    }
    else
    {
        return(NULL);
...
static void* _MacRxAllocCallback( size_t nitems, size_t size, void* param )
{
    ...
    if(_iRxEthDNode < (EMAC_RX_DESCRIPTORS+1))
    {
        return(&_rgRxEthDNode[_iRxEthDNode++]);
    }
    else
    {
        return(NULL);
