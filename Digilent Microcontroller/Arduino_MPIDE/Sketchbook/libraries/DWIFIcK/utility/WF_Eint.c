...
void __attribute((interrupt(ipl3), vector(_EXTERNAL_2_VECTOR), nomips16)) _WFInterrupt(void)
#elif defined(MRF24WB0M_IN_SPI_INT3)
void __attribute((interrupt(ipl3), vector(_EXTERNAL_3_VECTOR), nomips16)) _WFInterrupt(void)
#else
#error Must specify an SPI interrupt for WiFi
#endif
{
    // clear EINT
    if (WF_INT_IF && WF_INT_IE)
